...
const SurvivorOSTerminal = () => {
  ...
  const [log76Content, setLog76Content] = useState("");
  ...
  useEffect(() => {
    async function loadLog76() {
      try {
        const logResponse = await fetch('data/log76.md');
        const logContent = await logResponse.text();
        setLog76Content(logContent);
      } catch (error) {
        console.error("Error loading log76:", error);
      }
    }
...
To view a log, type 'log' followed by the log number (e.g., 'log76')`;
  ...
  const showLog76 = () => {
    addToTerminalHistory({ type: 'input', text: `> log76` });
    
    addToTerminalHistory({ 
      type: 'output', 
      text: 'ACCESSING LOG ENTRY #76',
    });
