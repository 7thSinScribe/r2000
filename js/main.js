...
const SurvivorOSTerminal = () => {
  ...
  // Survivor logs data - will be loaded from JSON file
  const [survivorLogsData, setSurvivorLogsData] = useState({
    availableLogs: [],
    logContent: {}
  });

  // Quickstart content - will be loaded from markdown file
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // Load logs index
        const logsResponse = await fetch('data/logs_index.json');
        const logsData = await logsResponse.json();
        setSurvivorLogsData(prevState => ({
          ...prevState,
          availableLogs: logsData.logs
        }));
  ...
  const showSurvivorLogs = () => {
    ...
    const sortedLogs = [...survivorLogsData.availableLogs].sort((a, b) => a.day - b.day);
    let logsText = `
...
`;
    ...
    logsText += `
To view a log, type the day number (e.g. "287")
...
`;
  ...
  const showSpecificLog = async (day) => {
    ...
    if (isNaN(dayNumber) || !survivorLogsData.availableLogs.find(log => log.day === dayNumber)) {
      addToTerminalHistory({ 
        type: 'output', 
